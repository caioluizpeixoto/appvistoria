...
with open(r'lib\core\services\fipe_service.dart', 'r', encoding='utf-8') as f:
    content = f.read()

new_fipe_service = """import 'dart:convert';
  ...
  static Future<FipeVeiculoInfo?> consultarFipe({
    ...
    try {
      ...
      for (final m in marcas) {
        ...
        if ((cleanMarca.contains('volkswagen') || cleanMarca == 'vw') &&
      ...
      for (final mod in modelos) {
        ...
        // Distinção E-Tech vs Z.E.
        final isModEtech = nomeMod.contains('tech') || nomeMod.contains('etech');
        final isModZe = nomeMod.contains('z e') || nomeMod.contains('ze');
        if (isQueryEtech && isModEtech) score += 20.0;
        if (isQueryEtech && isModZe) score -= 30.0;
        if (isQueryZe && isModZe) score += 20.0;
        if (isQueryZe && isModEtech) score -= 30.0;

        // Combustível / Versão
        if (isQueryFlex && nomeMod.contains('flex')) score += 10.0;
        if (isQueryDiesel && nomeMod.contains('diesel')) score += 10.0;
        if (isQueryHybrid && (nomeMod.contains('hybrid') || nomeMod.contains('hibrid'))) score += 15.0;
...
with open(r'lib\core\services\fipe_service.dart', 'w', encoding='utf-8') as f:
    f.write(new_fipe_service)

print('Updated fipe_service.dart successfully!')

...
class PdfGeneratorService {
  ...
  Future<String?> generateLaudoCompleto({
    ...
    try {
      ...
      if (consulta != null &&
        ...
        final radarPages =
            await PdfRadarGenerator.buildRadarPages(dadosPesquisa);
          ...
          children: [
            ...
            pw.Expanded(
              child: pw.Column(
                ...
                children: [
                  _disclaimerText(
                      'Este laudo trata-se da vistoria cautelar do veículo, possuindo caráter informativo da análise de itens, conforme padrões estabelecidos pelos fabricantes.',
                      styles),
                  _disclaimerText(
                      'NÃO substituindo em nenhuma hipótese a Perícia Oficial do Instituto de Criminalística.',
                      styles,
                      bold: true),
                  _disclaimerText(
                      'Cabe destacar que a unidade não se responsabiliza por quaisquer modificações nos itens do veículo contemplados nesta vistoria, posteriores à realização deste laudo, cuja validade tem sua garantia certificada no momento da realização da vistoria.',
                      styles),
                  _disclaimerText(
                      'O resultado do laudo técnico segue critérios de avaliação estabelecidos, podendo sofrer alterações necessárias em determinado momento, sem prévia comunicação.',
                      styles),
                  _disclaimerText(
                      'As informações dos veículos, obtidas através de pesquisa via base de dados dos órgãos públicos e empresas privadas, são de responsabilidade da empresa fornecedora da pesquisa, cabendo apenas reiterar os dados cadastrados nas referidas bases de consulta.',
                      styles),
                  _disclaimerText(
                      'Ao receber este laudo, o cliente fica ciente que as companhias de seguro possuem métodos e critérios próprios de avaliação do risco para aceitação ou não de veículos.',
                      styles),
                  _disclaimerText(
                      'Não obstante, o critério de avaliação bem como o resultado final da vistoria, independe da aceitação ou não da seguradora.',
                      styles),
                  _disclaimerText(
                      'Importante notar que NÃO são examinados itens de mecânica, elétrica, transmissão, suspensão e freios.',
                      styles,
                      bold: true),
                  _disclaimerText(
                      'A análise de pintura é realizada através de medidores digitais que informam a espessura da camada de tinta, apenas em caráter informativo de retoques ou reparos expressivos em sua lataria, que não afetam a estrutura do veículo, NÃO apontamos pequenos riscos nem desgastes na pintura.',
                      styles),
                  _disclaimerText(
                      'NÃO nos responsabilizamos por defeitos ou fraudes em equipamentos de Air-Bag.',
                      styles,
                      bold: true),
                  _disclaimerText(
                      'A verificação da numeração da caixa de câmbio somente é realizada se tal item está aparente, sem a necessidade de desmontar partes do veículo que tornem a gravação obstruída.',
                      styles),
                  _disclaimerText(
                      'A vistoria cautelar não afere a idoneidade da quilometragem constante no hodômetro do veículo, sendo apenas registrado em caráter informativo a quilometragem aparente em seu painel de instrumentos.',
                      styles),
                  _disclaimerText(
                      'Alguns itens obrigatórios e acessórios como pneus, setas, cintos e outros acessórios, são apenas informados quanto à sua existência e funcionamento mínimo, não sendo atestada calibração ou cumprimento de normas técnicas específicas.',
                      styles),
                  _disclaimerText(
                      'A análise proposta é particular, restrita exclusivamente aos itens analisados e não à vistoria regulamentada pelo CONTRAN ou à perícia realizada pelo Instituto de Criminalística.',
                      styles),
                  if (vistoria.tipoVistoria.toLowerCase().contains('caminh'))
                    _disclaimerText(
                        'ATENÇÃO (VEÍCULO DE CARGA): As modificações nas dimensões do chassi (alongamento ou encurtamento) estão sujeitas à regulamentação específica (Resolução CONTRAN nº 292/2008 e atualizações como a Res. 916/2022). Este laudo aponta apenas as condições aparentes da estrutura para fins de vistoria cautelar, não suprindo a exigência legal do Certificado de Segurança Veicular (CSV) nem garantindo a regularização das modificações perante o DETRAN.',
                        styles,
                        bold: true),
                ],
              ),
                ...
                children: [
                  if (assinaturaCliente != null)
                    pw.Container(
                      height: 30,
                      child:
                          pw.Image(assinaturaCliente, fit: pw.BoxFit.contain),
                    )
                  else
                    pw.SizedBox(height: 30),
                  pw.Container(width: 150, height: 1, color: _kBlack),
                            ...
                            pw.Wrap(
                              ...
                              children: [
                                ...
                                buildBlock(
                                    ...
                                    svgCalendar),
                                buildBlock('RECALL',
                                    hasRecall ? 'red' : 'green', svgCarWrench),
  ...
  pw.Widget _buildCategoryColumn(
    ...
    final double itemFontSize = total > 18 ? 5.5 : 7.5;
    final double itemBottomPadding = total > 18 ? 1.5 : 4.0;
    ...
    return pw.Expanded(
        child: pw.Padding(
            ...
            child: pw.Column(children: [
              ...
              pw.SizedBox(height: 8),
              _buildDonutChart(total, greens, yellows, reds, styles, limeGreen,
              ...
              pw.Container(
                  ...
                  child: pw.Column(
                      ...
                      children: itens.map((e) {
                        ...
                        return pw.Padding(
                            padding:
                                pw.EdgeInsets.only(bottom: itemBottomPadding),
                            child: pw.Row(
                                crossAxisAlignment:
          ...
          widgets.add(pw.Container(
            ...
            child: pw.Column(
              children: ids.asMap().entries.map((itemEntry) {
                ...
                final status = rawStatus.isEmpty ? 'OK' : rawStatus.toUpperCase();

                PdfColor itemStatusColor = PdfColors.green;
                String itemIcon = '<svg viewBox="0 0 24 24"><path fill="white" d="M9 16.2L4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4L9 16.2z"/></svg>';
                
                if (status.contains('COLISÃO') || status.contains('SOLDADO') || status.contains('SUBSTITUÍDO') || status.contains('DANIFICADO') || status.contains('OBSTRUÍDO') || status.contains('NÃO CONFORME')) {
                  itemStatusColor = PdfColor.fromHex('EE4036'); // Red
                  itemIcon = '<svg viewBox="0 0 24 24"><path fill="white" d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12 19 6.41z"/></svg>';
                } else if (status.contains('REPARO') || status.contains('OBSERVAÇÃO') || status.contains('ALONGADO') || status.contains('CONSIDERAÇÃO')) {
                  itemStatusColor = PdfColor.fromHex('FBB03B'); // Yellow
                  itemIcon = '<svg viewBox="0 0 24 24"><path fill="white" d="M1 21h22L12 2 1 21zm12-3h-2v-2h2v2zm0-4h-2v-4h2v4z"/></svg>';
                }
                ...
                return pw.Container(
                  ...
                  child: pw.Row(
                    ...
                    children: [
                      ...
                      pw.Row(
                        children: [
                          pw.Text(status, style: pw.TextStyle(font: styles.bold, fontSize: 7, color: itemStatusColor)),
                          pw.SizedBox(width: 4),
                          pw.Container(
                            width: 10, height: 10,
                            decoration: pw.BoxDecoration(color: itemStatusColor, shape: pw.BoxShape.circle),
                            child: pw.Center(child: pw.SvgImage(svg: itemIcon, width: 6, height: 6))
                          ),
                        ]
                      ),
                    ]
  ...
  PdfColor _getPinturaColor(String status) {
    ...
    if (status.contains('ORIGINAL') || status.contains('CONFORME') || status.contains('PADRÃO DO FABRICANTE')) {
      return _kGreen;
